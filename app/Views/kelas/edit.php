...
<div class="container p-4">
  <form action="/kelas/update/<?= $result['id']?>" method="post">
    <div class="row">
      <div class="col">
        <div class="card shadow bg-secondary-subtle">
          <div class="container p-4">
            ...
            <div class="form-group">
              <label for="tahun_ajaran" class="col-sm-2 position-relative form-label">Tahun Ajaran :</label>
              <div class="col-sm-10 position-relative">
                      ...
                      for ($year = $startYear; $year <= $currentYear; $year++) {
                        ...
                        ?>
                                  <option value="<?=$year .'/'. ($year + 1)?>"
                                    <?= old('tahun_ajaran') == $year ? 'selected' : ($result['tahun_ajaran'] == $tahunAjaran ? 'selected' : '') ?>>
                                    <?=$tahunAjaran?>
                                  </option>
                                  <?php
                      }
                ...
                <?php if (session()->has('validation') && session('validation')->getError('tahun_ajaran')): ?>
                <span class="text-danger ms-2" style="display:block; font-size: 12px">
                  <?= session('validation')->getError('tahun_ajaran');?>
                </span>
                <?php endif;?>
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <div class="input-wrapper">
                <input type="text" name="jumlah_siswa" class="form-control"
                  value="<?=old('jumlah_siswa') ?: $result['jumlah_siswa']?>">
                <?php if (session()->has('validation') && session('validation')->getError('jumlah_siswa')): ?>
                <span class="text-danger" style="display: block; font-size: 12px;">
                  <?= session('validation')->getError('jumlah_siswa'); ?>
                </span>
                <?php endif;?>
              </div>
            </div>
              ...
              <div class="col-sm-10 position-relative">
                <div class="input-wrapper position-relative">
                  <select name="id_walikelas" class="form-control">
                    ...
                    <option value="<?= $pilihGuru['id'] ?>"
                      <?= (old('id_walikelas') == $pilihGuru['id'] || (isset($result['wali_kelas']['id_walikelas']) && $result['wali_kelas']['id_walikelas'] == $pilihGuru['id'])) ? 'selected' : ''; ?>>
                      <?= $pilihGuru['nama_guru'] ?>
                    </option>
                    <?php endforeach;?>
                  </select>
                ...
                <span class="text-danger ms-2" style="display:block; font-size: 12px">
                  <?= session('validation')->getError('id_walikelas'); ?>
                </span>
                <?php endif;?>
              </div>
            </div>
